...
class StageTransitionView(discord.ui.View):
    ...
    async def proceed(
        ...
        stage = await self.database.get_member_stage(
...
class VerificationView(discord.ui.View):
    ...
    async def verify(
        ...
        status = await db.verification_status(
            interaction.guild.id,
            interaction.user.id,
        )
        ...
        await db.set_verification_status(
            interaction.guild.id,
            interaction.user.id,
            "pending",
        )

        await db.set_member_stage(
            interaction.guild.id,
            interaction.user.id,
            "pending_verify",
        )
        ...
        req_embed = hx_embed(
            "ACCESS REQUEST // TX",
            f"{CYN}[MEMBER]{RST} {interaction.user.mention}\n"
            f"{CYN}[NAME]{RST}   {interaction.user.display_name}\n"
            f"{CYN}[UID]{RST}    `{interaction.user.id}`\n"
        ...
        req_embed.set_footer(
            text=f"member:{interaction.user.id}"
        )

        await queue.send(
            embed=req_embed,
            view=ApprovalView(db),
        )
        ...
        if interaction.guild.owner:
            try:
                await interaction.guild.owner.send(
                    f"{CYN}[ALERT]{RST} New access request in "
                    f"**{interaction.guild.name}** from "
        ...
        if footer.startswith("clearance:"):
            ...
            if approved and member and role:
                if role >= interaction.guild.me.top_role:
                    return await interaction.response.send_message(
                        "Move my role above this clearance role before approving.",
                        ephemeral=True,
                    )

                await member.add_roles(
                    role,
                    reason=f"Clearance approved by {interaction.user}",
                )

                await self.database.set_member_stage(
                    interaction.guild.id,
                    member_id,
                    "cleared",
                )
                ...
                if arrival:
                    tx_embed = hx_embed(
                        "CLEARANCE UPGRADED // SECTOR 02 READY",
            ...
            if approved and member:
                ...
                if role is None or role >= interaction.guild.me.top_role:
                    return await interaction.response.send_message(
                        "Move my role above Verified before approving.",
                        ephemeral=True,
                    )

                await member.add_roles(
                    role,
                    reason=f"Access approved by {interaction.user}",
                )
            ...
            await self.database.set_verification_status(
                interaction.guild.id,
                member_id,
                "approved" if approved else "rejected",
            )
...
class RoleView(discord.ui.View):
    ...
    async def _check_and_request(
        ...
        previous = await self.database.clearance_status(
            interaction.guild.id,
            interaction.user.id,
        )

        if role in interaction.user.roles or (
            previous and previous[1] == "approved"
        ):
            return await interaction.response.send_message(
                "You already hold this clearance.",
                ephemeral=True,
            )
        ...
        req_embed.set_footer(
            text=f"clearance:{interaction.user.id}:{role_name}"
        )

        await queue.send(
            embed=req_embed,
            view=ApprovalView(self.database),
        )
...
class RoomPanelView(discord.ui.View):
    ...
    async def begin(
        ...
        await interaction.response.send_modal(
            RoomModal(self.database)
        )


class RoomModal(
    discord.ui.Modal,
    title="Configure your temporary room",
):
    ...
    async def on_submit(
        ...
        try:
            duration = max(
                ...
                min(
                    720,
                    int(str(self.minutes)),
                ),
            )
        except ValueError:
            return await interaction.response.send_message(
                "Duration must be a number from 5 to 720.",
                ephemeral=True,
            )

        await interaction.response.send_message(
            "Choose the room privacy:",
            view=RoomPrivacyView(
                self.database,
                str(self.room_name),
                duration,
            ),
...
class RoomPrivacyView(discord.ui.View):
    ...
    async def create(
        ...
        channel = await guild.create_voice_channel(
            ...
            reason=f"Temporary room created by {interaction.user}",
        )
...
class TicketView(discord.ui.View):
    ...
    async def open_ticket(
        ...
        previous = await self.database.open_ticket_for(
            guild.id,
            interaction.user.id,
        )
        ...
        if category is None or staff is None:
            return await interaction.followup.send(
                "Ticket system not configured. Run /setup.",
                ephemeral=True,
            )
        ...
        await interaction.followup.send(
            f"Secure channel created: {channel.mention}",
            ephemeral=True,
        )
...
class CloseTicketView(discord.ui.View):
    ...
    async def close_ticket(
        ...
        owner = await self.database.ticket_owner(
        ...
        await interaction.response.send_message(
            "Ticket will close in five seconds.",
            ephemeral=True,
        )

        await self.database.close_ticket(
            interaction.channel.id
        )
...
class ReactionRoleCreateView(discord.ui.View):
    ...
    async def create_rr(
        ...
        await interaction.response.send_modal(
            ReactionRoleModal(self.database)
        )


class ReactionRoleModal(
    discord.ui.Modal,
    ...
    async def on_submit(
        ...
        await self.database.add_reaction_role(
            interaction.guild.id,
            msg_id,
            str(self.emoji),
            role.id,
        ...
        await interaction.response.send_message(
            f"{GRN}[OK]{RST} Reaction role created: "
            f"{self.emoji} → {role.mention}",
            ephemeral=True,
        )
...
class SuggestionVoteView(discord.ui.View):
    ...
    async def upvote(
        ...
        if current_vote == 1:
            return await interaction.response.send_message(
                "You have already upvoted this suggestion.",
                ephemeral=True,
            )
    ...
    async def downvote(
        ...
        if current_vote == -1:
            return await interaction.response.send_message(
                "You have already downvoted this suggestion.",
                ephemeral=True,
            )
...
class AutoModConfigView(discord.ui.View):
    ...
    async def toggle_spam(
        ...
        config = await self.database.get_automod(
            interaction.guild.id
        )
        ...
        await self.database.set_automod(
            interaction.guild.id,
            new_val,
            config[2] if config else 0,
            config[3] if config else 0,
            config[4] if config else 5,
            config[5] if config else 300,
        )
    ...
    @discord.ui.button(
        label="Toggle Anti-Caps",
        emoji="🔤",
        style=discord.ButtonStyle.primary,
        custom_id="nexus:am:caps",
    )
    async def toggle_caps(
        self,
        interaction: discord.Interaction,
        _: discord.ui.Button,
    ) -> None:
