...
# ── ANSI colour palette for Discord terminal blocks ──
BLK = "\u001b[0;30m"   # dark grey
RED = "\u001b[1;31m"   # red
GRN = "\u001b[1;32m"   # green
YLW = "\u001b[1;33m"   # yellow
...
def panel_embed(title: str, body: str) -> discord.Embed:
    """Legacy helper — kept for non-terminal panels."""
    return discord.Embed(title=title, description=body, colour=ACCENT, timestamp=datetime.now(UTC))


def hx_embed(header: str, lines: str, colour: int = 0x00FF41) -> discord.Embed:
    """Hacker-style terminal embed with ANSI frame."""
    frame = (
        f"{CYN}╔══════════════════════════════════════════════════════╗{RST}\n"
        f"{CYN}║{RST}  {WHT}{header:^50}{RST}  {CYN}║{RST}\n"
        f"{CYN}╠══════════════════════════════════════════════════════╣{RST}\n"
        f"{lines}\n"
        f"{CYN}╚══════════════════════════════════════════════════════╝{RST}"
    )
...
class StageTransitionView(discord.ui.View):
    """Persistent view used for every stage-transition message.
    The button reads the user\'s current stage from the DB and
    replies with the correct next-sector embed + controls."""
    ...
    async def proceed(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        if stage == "verified":
            embed = hx_embed(
                ...
                f"{WHT}  [1] Elite Agent  — Full system access{RST}\n"
                f"{WHT}  [2] Guest Node   — Limited read-only{RST}",
                colour=0x7C3AED,
            )
...
class VerificationView(discord.ui.View):
    ...
    async def verify(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        if status == "approved":
            return await interaction.response.send_message("Your access is already approved.", ephemeral=True)
        if status == "pending":
            return await interaction.response.send_message("Your verification request is already waiting for review.", ephemeral=True)

        await self.database.set_verification_status(interaction.guild.id, interaction.user.id, "pending")
        await self.database.set_member_stage(interaction.guild.id, interaction.user.id, "pending_verify")
        ...
        # Alert owner via DM when possible
        if interaction.guild.owner:
            try:
                await interaction.guild.owner.send(
                    f"{CYN}[ALERT]{RST} New access request in **{interaction.guild.name}** from {interaction.user} (`{interaction.user.id}`). Review it in 🔐-approval-queue."
                )
            except discord.Forbidden:
                pass

        await interaction.response.send_message(
            f"{GRN}[OK]{RST}  Request transmitted. Await administrator clearance.", ephemeral=True
        )
            ...
            if approved and member and role:
                ...
                if arrival:
                    tx_embed = hx_embed(
                        ...
                        f"{BLK}[TIME]{RST}  {datetime.now(UTC).strftime('%H:%M:%S')} UTC",
                        colour=0xFFD700,
                    )
...
class RoleView(discord.ui.View):
    ...
    async def request(self, interaction: discord.Interaction, role_name: str) -> None:
        ...
        embed = hx_embed(
            ...
            f"{GRN}[STATUS]{RST}  Pending owner review...",
            colour=0x7C3AED,
        )
        embed.set_footer(text=f"clearance:{interaction.user.id}:{role_name}")
        await queue.send(embed=embed, view=ApprovalView(self.database))
        await interaction.response.send_message(
            f"{GRN}[OK]{RST}  {role_name} clearance request submitted for owner approval.", ephemeral=True
        )
