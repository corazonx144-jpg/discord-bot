...
# ANSI helpers for on-the-fly strings in bot.py
CYN = "\u001b[1;36m"
GRN = "\u001b[1;32m"
YLW = "\u001b[1;33m"
RED = "\u001b[1;31m"
...
class NexusBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        self.add_view(StageTransitionView(self.database))  # NEW: stage-transition buttons survive restarts
...
def arrival_terminal_embed(guild: discord.Guild) -> discord.Embed:
    return hx_embed(
        ...
        f"{CYN}[>>]{RST}   Next: Execute verification protocol\n\n"
        f"{WHT}01 · REQUEST ACCESS{RST}\n"
        f"{BLK}     Submit identity for admin review{RST}\n\n"
        f"{WHT}02 · AWAIT CLEARANCE{RST}\n"
        f"{BLK}     No automatic role grants{RST}",
        colour=0x22D3EE,
    )
...
async def build_layout(guild: discord.Guild) -> None:
    ...
    if guild.owner:
        owner_overwrite[guild.owner] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True)
    ...
    await ensure_panel(arrival, "arrival", arrival_terminal_embed(guild), VerificationView(bot.database))
    await ensure_panel(verify, "verification", hx_embed("ACCESS // STAGE 01", f"{CYN}[INFO]{RST}  Transmit an access request.\n{YLW}[WARN]{RST} Approval opens the Clearance Gate only."), discord.ui.View())
    await ensure_panel(roles, "roles", hx_embed("CLEARANCE // STAGE 02", f"{CYN}[INFO]{RST}  Request one clearance.\n{YLW}[WARN]{RST} Owner must approve before final sections unlock."), RoleView(bot.database))
    await ensure_panel(tickets, "tickets", hx_embed("SUPPORT TERMINAL", f"{GRN}[OK]{RST}  Open one private, persistent support ticket."), TicketView(bot.database))
    await ensure_panel(room_panel, "room_generator", hx_embed("VOICE LAB", f"{CYN}[INFO]{RST}  Create a public or private voice room.\n{YLW}[WARN]{RST} Self-destructs on owner exit or timer expiry."), RoomPanelView(bot.database))
...
async def on_member_join(member: discord.Member) -> None:
    ...
    if channel:
        embed = hx_embed(
            ...
            f"{RED}[ACTION]{RST}  Initiate verification protocol immediately",
            colour=0xFF4444,
        )
