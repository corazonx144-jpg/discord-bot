...
# ── Whitelist for clean setup ──
_ALLOWED_CATEGORIES = {
    "🔒 ─ SECTOR 01 │ SYSTEM CORE", "🧬 ─ SECTOR 02 │ CLEARANCE GATE",
    "⚡ ─ SECTOR 03 │ TERMINAL CHAT", "🎧 ─ SECTOR 04 │ SECURE NODES",
    "🎛️ ─ SECTOR 05 │ SERVICES", "📁 ─ SECTOR 06 │ ARCHIVE",
    "👁️ ─ SECTOR 07 │ CONTROL", "🧪 ─ DEV & MOD OPS",
    "👑 ─ VIP & CREATIVE", "🚪 ─ SECTOR 08 │ DEPARTURE",
    # Legacy names for detection
    "🔒 -- [ SECTOR 01 ] SYSTEM CORE", "🧬 -- [ SECTOR 02 ] CLEARANCE GATE",
    "⚡ -- [ SECTOR 02 ] TERMINAL CHAT", "⚡ ─ SECTOR 02 │ TERMINAL CHAT",
    "🎧 -- [ SECTOR 03 ] SECURE NODES", "🎧 ─ SECTOR 03 │ SECURE NODES",
    "📁 -- [ SECTOR 04 ] INTELLIGENCE ARCHIVE", "📁 ─ SECTOR 04 │ INTELLIGENCE ARCHIVE",
    "📁 ─ SECTOR 05 │ INTELLIGENCE ARCHIVE",
    "👁️ -- [ SECTOR 05 ] CONTROL & LOGS", "👁️ ─ SECTOR 05 │ CONTROL & LOGS",
    "👁️ ─ CONTROL & LOGS",
}

_ALLOWED_TEXT = {
    "⌁-arrival-terminal", "🛡️-verify-access", "📜-protocol-rules",
    "📖-user-guide", "🔒-admin-manual", "⚡-select-clearance",
    "🌐-global-network", "💻-command-shell", "⭐-starboard",
    "📊-community-hub", "🎛️-create-your-room", "🎫-open-a-ticket",
    "📁-mission-briefs", "🏆-leaderboards", "📊-surveillance-logs",
    "🔐-approval-queue", "👁️-control-panels", "🧪-dev-terminal",
    "👑-vip-lounge", "🎨-art-gallery", "🎵-music-lab", "📺-stream-deck",
    "🚪-departure-terminal",
    # Legacy text channels to delete
    "📡-broadcasts", "📋-verification-logs", "📋-support-protocol",
    "📊-data-logs", "📊-polls", "💡-suggestions", "🏆-leaderboard",
    "💰-economy-leaderboard", "🤖-automod-config", "🎭-reaction-roles",
    "🛡️-mod-ops", "📋-case-files", "🎁-booster-perks",
}

_ALLOWED_VOICE = {
    "🔒 Node 01 • Safe Zone", "🛡️ Node 02 • Operations", "⚡ Node 03 • Alpha",
}
...
async def _clean_layout(guild: discord.Guild) -> None:
    """Remove channels and roles not in the whitelist."""
    me = guild.me
    if not me or not me.guild_permissions.manage_channels or not me.guild_permissions.manage_roles:
        return

    # ── Delete unknown text/voice channels ──
    for channel in list(guild.channels):
        if isinstance(channel, discord.CategoryChannel):
            continue
        name = channel.name
        if isinstance(channel, discord.TextChannel):
            if name not in _ALLOWED_TEXT:
                with suppress(discord.Forbidden, discord.NotFound):
                    await channel.delete(reason="Nexus clean setup: channel not in whitelist")
        elif isinstance(channel, discord.VoiceChannel):
            if name not in _ALLOWED_VOICE:
                # Check if it's a temp room (in DB)
                record = await bot.database.room(channel.id)
                if not record:
                    with suppress(discord.Forbidden, discord.NotFound):
                        await channel.delete(reason="Nexus clean setup: channel not in whitelist")

    # ── Delete unknown categories ──
    for category in list(guild.categories):
        if category.name not in _ALLOWED_CATEGORIES:
            # Only delete if empty or all children are being deleted
            with suppress(discord.Forbidden, discord.NotFound):
                await category.delete(reason="Nexus clean setup: category not in whitelist")

    # ── Delete unknown bot-managed roles ──
    for role in list(guild.roles):
        if role.name not in _ALLOWED_ROLES and not role.managed and role != guild.default_role and role != me.top_role:
            if role.name in ("Muted",):  # Keep AutoMod mute role if exists
                continue
            with suppress(discord.Forbidden):
                await role.delete(reason="Nexus clean setup: role not in whitelist")

async def build_layout(guild: discord.Guild) -> None:
    # Clean up old channels/roles first
