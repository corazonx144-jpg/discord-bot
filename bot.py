...
async def ensure_stage(guild: discord.Guild, name: str, category: discord.CategoryChannel = None) -> discord.StageChannel | None:
    ...
    if channel is None:
        try:
            ...
            log.info("[build_layout] Created stage channel: %s", name)
        except discord.HTTPException as e:
            log.error("[build_layout] Failed to create stage channel %s: %s (code: %s)", name, e, e.code if hasattr(e, 'code') else 'N/A')
            return None
    return channel
...
async def build_layout(guild: discord.Guild) -> None:
    ...
    log.info("[build_layout] Starting layout build for guild: %s", guild.name)

    try:
        await _clean_layout(guild)
    except Exception as e:
        log.error("[build_layout] _clean_layout failed: %s", e, exc_info=True)
        raise

    log.info("[build_layout] _clean_layout done")

    # ── Neon Roles ──
    log.info("[build_layout] Creating roles...")
    r = {
    ...
    log.info("[build_layout] Roles created")
    ...
    log.info("[build_layout] Creating SECTOR 00...")
    ...
    log.info("[build_layout] SECTOR 00 done")

    log.info("[build_layout] Creating SECTOR 01...")
    ...
    log.info("[build_layout] SECTOR 01 done. Creating SECTOR 02...")
    ...
    log.info("[build_layout] SECTOR 02 done. Creating SECTOR 03...")
    ...
    log.info("[build_layout] SECTOR 03 done. Creating SECTOR 04...")
    ...
    log.info("[build_layout] SECTOR 04 done. Creating SECTOR 05...")
    ...
    log.info("[build_layout] SECTOR 05 done. Creating SECTOR 06...")
    ...
    log.info("[build_layout] SECTOR 06 done. Creating SECTOR 07...")
    ...
    log.info("[build_layout] SECTOR 07 done. Creating SECTOR 08...")
    ...
    log.info("[build_layout] Creating stage channel...")
    stage_ch = await ensure_stage(guild, "📡-live-briefing", category=gateway)
    if stage_ch is None:
        log.warning("[build_layout] Stage channel creation skipped (server may not support it)")

    # ── Panels ──
    log.info("[build_layout] Sending panels...")
    ...
    log.info("[build_layout] Panels done. Layout complete!")
...
async def reset_cmd(interaction: discord.Interaction) -> None:
    ...
    try:
        ...
        log.error("[reset_cmd] Error during reset: %s", e, exc_info=True)
        await interaction.followup.send(f"❌ Reset failed: {type(e).__name__}: {str(e)[:500]}. Check Render logs for details.", ephemeral=True)
...
async def setup(interaction: discord.Interaction) -> None:
    ...
    try:
        ...
        log.error("[setup] Error during setup: %s", e, exc_info=True)
        await interaction.followup.send(f"❌ Setup failed: {type(e).__name__}: {str(e)[:500]}. Check Render logs for details.", ephemeral=True)
