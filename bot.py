...
async def build_layout(guild: discord.Guild) -> None:
    ...
    arrival = await ensure_text(gateway, "⌁-arrival-terminal")
    verify = await ensure_text(gateway, "🛡️-verify-access")
    rules_ch = await ensure_text(gateway, "📜-protocol-rules")
    user_guide = await ensure_text(gateway, "📖-user-guide")
    admin_manual = await ensure_text(gateway, "🔒-admin-manual")
    ...
    for name in ("🔒 Node 00 • Safe Zone", "🛡️ Node 01 • Operations", "⚡ Node 02 • Alpha"):
        await ensure_voice(nodes, name)

    # ── SECTOR 04 │ SERVICES (Verified+: Room Generator + Tickets for Support) ──
    ...
    surv_ch = await ensure_text(control, "📊-surveillance-logs")
    ...
    await ensure_text(vault, "💎-vip-lounge")
    await ensure_text(vault, "🎨-art-gallery")
    await ensure_text(vault, "🎵-music-lab")
    await ensure_text(vault, "📺-stream-deck")
