...
def user_guide_embed() -> discord.Embed:
    ...
    lines = (
        ...
        f"{CYN}╠══════════════════════════════════════════════════════════════╣{RST}\n"
        f"{GRN}[01]{RST}  /verify @user      {DIM}— Instant verify (Admin/Mod only){RST}\n"
        f"{GRN}[02]{RST}  /rank              {DIM}— Check your level & XP{RST}\n"
        f"{GRN}[03]{RST}  /leaderboard       {DIM}— Top 10 members by level{RST}\n"
        f"{GRN}[04]{RST}  /balance           {DIM}— Check your credits{RST}\n"
        f"{GRN}[05]{RST}  /daily             {DIM}— Claim daily reward (24h cooldown){RST}\n"
        f"{GRN}[06]{RST}  /pay @user amount  {DIM}— Send credits to someone{RST}\n"
        f"{GRN}[07]{RST}  /work              {DIM}— Work to earn credits{RST}\n"
        f"{GRN}[08]{RST}  /economyboard      {DIM}— Top 10 richest members{RST}\n"
        f"{GRN}[09]{RST}  /poll question ... {DIM}— Create a poll{RST}\n"
        f"{GRN}[10]{RST}  /suggest content   {DIM}— Submit a suggestion{RST}\n"
        f"{GRN}[11]{RST}  /8ball question    {DIM}— Magic 8-ball{RST}\n"
        f"{GRN}[12]{RST}  /coinflip          {DIM}— Flip a coin{RST}\n\n"
        f"{YLW}[TICKETS]{RST}\n"
        f"  Click 🎫 in 🎫-open-a-ticket to open a support ticket.\n\n"
        f"{YLW}[VOICE ROOMS]{RST}\n"
        f"  Click 🎛️ in 🎛️-create-your-room to make a temp voice room.\n\n"
        f"{YLW}[CLEARANCE]{RST}\n"
        f"  1. Press VERIFY.EXE in ⌁-arrival-terminal\n"
        f"  2. Wait for admin approval\n"
        f"  3. Go to ⚡-select-clearance and request a role\n"
        f"  4. Wait for admin approval again\n\n"
        f"{RED}[RULES]{RST} Read 📜-protocol-rules carefully."
    )
    return cmd_embed("USER MANUAL // V1.0", lines, colour=0x00FF41)
...
def admin_manual_embed() -> discord.Embed:
    ...
    lines = (
        ...
        f"{GRN}[01]{RST}  /warn @user reason       {DIM}— Issue a warning{RST}\n"
        f"{GRN}[02]{RST}  /warnings @user            {DIM}— View user warnings{RST}\n"
        f"{GRN}[03]{RST}  /clearwarns @user          {DIM}— Clear all warnings{RST}\n"
        f"{GRN}[04]{RST}  /kick @user reason         {DIM}— Kick member{RST}\n"
        f"{GRN}[05]{RST}  /ban @user reason days     {DIM}— Ban member (0-7 days delete){RST}\n"
        f"{GRN}[06]{RST}  /unban user_id             {DIM}— Unban by ID{RST}\n"
        f"{GRN}[07]{RST}  /timeout @user duration    {DIM}— Timeout (30m, 1h, 1d){RST}\n"
        f"{GRN}[08]{RST}  /purge amount @user(opt)   {DIM}— Mass delete messages{RST}\n"
        f"{GRN}[09]{RST}  /clear amount              {DIM}— Delete 1-100 messages{RST}\n\n"
        f"{RED}[ADMIN]{RST}\n"
        f"{YLW}[01]{RST}  /setup                     {DIM}— Create/repair server layout{RST}\n"
        f"{YLW}[02]{RST}  /verify @user              {DIM}— Instant verify member{RST}\n"
        f"{YLW}[03]{RST}  /automod                   {DIM}— Configure AutoMod{RST}\n"
        f"{YLW}[04]{RST}  /say #channel text         {DIM}— Bot says something{RST}\n"
        f"{YLW}[05]{RST}  /embed #channel title ...  {DIM}— Send custom embed{RST}\n"
        f"{YLW}[06]{RST}  /status                    {DIM}— Bot health check{RST}\n\n"
        f"{RED}[AUTOMOD]{RST}\n"
        f"  Toggle in 🤖-automod-config or /automod\n"
        f"  • Anti-Spam: Mutes spammer for 5 min\n"
        f"  • Anti-Link: Deletes all URLs\n"
        f"  • Anti-Caps: Deletes 70%+ caps messages\n\n"
        f"{RED}[LOGGING]{RST}\n"
        f"  All actions logged to 📊-surveillance-logs\n"
        f"  Cases saved in 📋-case-files\n\n"
        f"{RED}[REACTION ROLES]{RST}\n"
        f"  Use /reactionrole or panel in 🎭-reaction-roles"
    )
    return cmd_embed("ADMIN MANUAL // V1.0", lines, colour=0xFF4444)
...
async def build_layout(guild: discord.Guild) -> None:
    ...
    await ensure_text(terminal, "📊-polls")
    await ensure_text(terminal, "💡-suggestions")
    ...
    await ensure_text(dev_cat, "🤖-automod-config")
    ...
    intel = await ensure_category(guild, "📁 ─ SECTOR 07 │ INTELLIGENCE ARCHIVE", vbase, "📁 -- [ SECTOR 04 ] INTELLIGENCE ARCHIVE", "📁 ─ SECTOR 04 │ INTELLIGENCE ARCHIVE", "📁 ─ SECTOR 05 │ INTELLIGENCE ARCHIVE")
    await ensure_text(intel, "📁-mission-briefs")
    await ensure_text(intel, "📊-data-logs")
    await ensure_text(intel, "🏆-leaderboard")
    await ensure_text(intel, "💰-economy-leaderboard")
    ...
    control = await ensure_category(guild, "👁️ ─ SECTOR 08 │ CONTROL & LOGS", control_ow, "👁️ -- [ SECTOR 05 ] CONTROL & LOGS", "👁️ ─ SECTOR 05 │ CONTROL & LOGS", "👁️ ─ CONTROL & LOGS")
    ...
    # ── SECTOR 10 │ DOCUMENTATION (public read-only) ──
    docs_ow = {
        guild.default_role: discord.PermissionOverwrite(view_channel=True, send_messages=False, read_message_history=True),
        me: discord.PermissionOverwrite(view_channel=True, send_messages=True, manage_channels=True, read_message_history=True),
    }
    docs = await ensure_category(guild, "📚 ─ SECTOR 10 │ DOCUMENTATION", docs_ow)
    user_guide_ch = await ensure_text(docs, "📖-user-guide")
    admin_manual_ch = await ensure_text(docs, "🔒-admin-manual")

    await ensure_text(control, "🎭-reaction-roles")

    # ── SECTOR 09 │ DEPARTURE (Verified+) ──
    depart_ow = dict(vbase)
    depart = await ensure_category(guild, "🚪 ─ SECTOR 09 │ DEPARTURE", depart_ow)
    ...
    await ensure_panel(user_guide_ch, "user_guide", user_guide_embed(), discord.ui.View())
    await ensure_panel(admin_manual_ch, "admin_manual", admin_manual_embed(), discord.ui.View())


    # AutoMod panel
    am_ch = discord.utils.get(guild.text_channels, name="🤖-automod-config")
    if am_ch:
        await ensure_panel(am_ch, "automod", cmd_embed("AUTOMOD CONFIG",
    ...
    rr_ch = discord.utils.get(guild.text_channels, name="🎭-reaction-roles")
    if rr_ch:
        await ensure_panel(rr_ch, "reaction_roles", cmd_embed("REACTION ROLES",
            f"{CYN}[INFO]{RST}  Staff: use /reactionrole to create.\n"
            f"{DIM}Members react to get roles automatically.{RST}"), ReactionRoleCreateView(bot.database))
