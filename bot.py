...
def _verified_base_overwrites(guild: discord.Guild, me: discord.Member, roles: dict) -> dict:
    """Base overwrites for Verified+ channels."""
    o = {
...
async def build_layout(guild: discord.Guild) -> None:
    ...
    clearance = await ensure_category(guild, "🧬 ─ SECTOR 02 │ CLEARANCE GATE", vbase)
    roles_ch = await ensure_text(clearance, "⚡-select-clearance")
    await ensure_text(clearance, "📋-verification-logs")

    # ── SECTOR 03 │ TERMINAL CHAT (Verified+ with role-specific rooms) ──
    terminal = await ensure_category(guild, "⚡ ─ SECTOR 03 │ TERMINAL CHAT", vbase)
    ...
    secure = await ensure_category(guild, "🎧 ─ SECTOR 04 │ SECURE NODES", vbase)
    ...
    await ensure_text(support_cat, "🎫-open-a-ticket")
    await ensure_text(support_cat, "📋-support-protocol")

    # ── SECTOR 07 │ INTELLIGENCE ARCHIVE (Verified+) ──
    intel = await ensure_category(guild, "📁 ─ SECTOR 07 │ INTELLIGENCE ARCHIVE", vbase)
    ...
    control = await ensure_category(guild, "👁️ ─ SECTOR 08 │ CONTROL & LOGS", control_ow)
    ...
    await ensure_panel(support_cat.text_channels[0], "tickets", hx_embed("SUPPORT TERMINAL",
...
async def setup(interaction: discord.Interaction) -> None:
    ...
    terminal = await ensure_category(guild, "⚡ ─ SECTOR 03 │ TERMINAL CHAT", vbase, "⚡ -- [ SECTOR 02 ] TERMINAL CHAT")
    await ensure_text(terminal, "🌐-global-network")
    await ensure_text(terminal, "💻-command-shell")
    await ensure_text(terminal, "📦-payload-archive")
    ...
    secure = await ensure_category(guild, "🎧 ─ SECTOR 04 │ SECURE NODES", vbase, "🎧 ─ SECTOR 03 │ SECURE NODES")
    for name in ("🔒 Node 01 • Safe Zone", "🛡️ Node 02 • Operations", "⚡ Node 03 • Alpha"):
        await ensure_voice(secure, name)
    ...
    intel = await ensure_category(guild, "📁 ─ SECTOR 07 │ INTELLIGENCE ARCHIVE", vbase, "📁 ─ SECTOR 04 │ INTELLIGENCE ARCHIVE")
    await ensure_text(intel, "📁-mission-briefs")
    await ensure_text(intel, "📊-data-logs")
    ...
    await interaction.followup.send("Nexus layout is ready. All sectors, roles and channels deployed.", ephemeral=True)
